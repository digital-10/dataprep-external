...
public class Job3ExportSingle extends CustomQuartzJobBean {
	...
	public void executeInternal(JobExecutionContext context) throws JobExecutionException {
    	if(context.getJobDetail().getJobDataMap().getBoolean("job3IsRunningPossible")) {
    		LOGGER.info("QuartzJobListner - Job3 BEGIN");
			...
			if(StringUtils.isNotBlank(jsonStr)) {
				...
				// 4. 복제 대상 preparation 정보가 없는 파일의 제거후 재저장. 
				if(flagInfoRewrite) {
...
*/								
	...
	private void exit() {
		...
		LOGGER.info("QuartzJobListner - Recipe Info Not Found");
	}
}
