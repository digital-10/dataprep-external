...
/*			
	        ...
	        hadoopUtil = new HadoopUtil().init();
	        dprepHttpUtil = new DprepHttpUtil().init();
	        //dprepUtil = DprepUtil(dprepHttpUtil, hadoopUtil).init();
	        dprepUtil = new DprepUtil(dprepHttpUtil, hadoopUtil).init();
	        dprepMetaUtil = new DprepMetaUtil().init();
*/	        			
		}
	...
	private void job4ToBeExecuted(JobExecutionContext context) {
    	...
    	List<String> jobStatusNode = (List<String>)springRedisTemplateUtil.valueGet("JOB_STATUS_NODE_" + nodeNo);
 		...
 		// job1, job2, job3 가 끝났을때 시작
 		if("DONE".equals(jobStatusNode.get(0)) && "DONE".equals(jobStatusNode.get(1)) && "DONE".equals(jobStatusNode.get(2))) {
 	 		if("NEW".equals(jobStatusNode.get(3))) {
 	 			jobStatusNode.set(3, "RUNNING");
 	 			
 	 			springRedisTemplateUtil.valueSet("JOB_STATUS_NODE_" + nodeNo, jobStatusNode);
 	 		}
 	 		else if("RUNNING".equals(jobStatusNode.get(3))) {
 	 			job4IsRunningPossible = false;
 	 		} 	 		
 	 		else if("DONE".equals(jobStatusNode.get(3))) {
 	 			job4IsRunningPossible = false;
 	 		} 	 		
 		}
 		else {
...
/*					
					String fileYear = dprepUtil.extractYearFromFileName(srcPath);					
					if(StringUtils.isNotBlank(fileYear)) {
						dstPath = dstPath + "/" + fileYear;
					}
*/					
				...
				//String prevSavedJson = hadoopUtil.readToStr((hadoopResultRegBasePath + "/" + yyyyMMdd.substring(0, 4) + "/" + yyyyMMdd.substring(4) + ".json"), "UTF-8");
				...
				// 하둡에 적재
				//hadoopUtil.write(fs, gson.toJson(prevExportResultVOs), hadoopResultRegBasePath, yyyyMMdd.substring(0, 4), yyyyMMdd.substring(4) + ".json");
				hadoopUtil.write(fs, gson.toJson(prevExportResultVOs), hadoopResultRegBasePath, wsId, "list.json");							
/*
				Set<String> export = new HashSet<>();
				if(StringUtils.isNotBlank(prevSavedJson)) {
					export = gson.fromJson(prevSavedJson, new TypeToken<Set<String>>() {}.getType());
				}
				...
				// 하둡에 적재
				FileSystem fs = hadoopUtil.getFs();
				hadoopUtil.write(fs, gson.toJson(export), hadoopResultRegBasePath, yyyyMMdd.substring(0, 4), yyyyMMdd.substring(4) + ".json");
...
*/
				...
				LOGGER.info("QuartzJobListner - All Job DONE");
...
/*				
	    		if(SchedulingController.flagStop) {
...
*/
				...
				LOGGER.info("QuartzJobListner - Job4 DONE");
    ...
    private void eachSaveActionsMetadata() {
		...
		//List<JsonArray> rowMetadataList = new ArrayList<>();
		...
		if(StringUtils.isNotBlank(jsonStrExportSingle)) {
			...
			for(int i = 0, len = exportSingle.size(); i < len; i++) {
				...
				//hadoopUtil.write(fs, gson.toJson(content), hadoopResultRegBasePath, String.format("/%s/action", wsId), String.format("%s_actions.json", processingInfomationVO.getFileNames().get(0)));
/*				
				JsonArray rowMetadata = dprepMetaUtil.getMetadata(rowMetadataId);
				rowMetadataList.add(rowMetadata);
				
				hadoopUtil.write(fs, gson.toJson(rowMetadata), hadoopResultRegBasePath, String.format("/%s/metadata", wsId), String.format("%s_metadata.json", processingInfomationVO.getFileNames().get(0)));
*/				
			}
		}
...
/*				
				...
				List<JsonArray> splitFileRowMetadataList = new ArrayList<>();
				for(int j = 0, jLen = preparationIds.size(); j < jLen; j++) {
					...
					JsonArray rowMetadata = dprepMetaUtil.getMetadata(rowMetadataId);
					splitFileRowMetadataList.add(rowMetadata);			
				}
				
				JsonArray mergeMeta = dprepMetaUtil.mergeMetadata(splitFileRowMetadataList);
				rowMetadataList.add(mergeMeta);

				hadoopUtil.write(fs, gson.toJson(mergeMeta), hadoopResultRegBasePath, String.format("/%s/metadata", wsId), String.format("%s_metadata.json", processingInfomationVO.getFileNames().get(0)));
*/								
			}
		}

		springRedisTemplateUtil.valueSet("ACTIONS_" + nodeNo, gson.toJson(contentList));
		//springRedisTemplateUtil.valueSet("METADATA_" + nodeNo, gson.toJson(rowMetadataList));
/*		
    	if(null != fs) {
    		try { fs.close(); } catch (IOException e) {}    		
    	}
*/    	
/* 이 행위는 0번 노드에서만 필요한듯하여...더 생각 필요...    	
    	this.mergeActions();
    	this.mergeMetadata();
*/    	
    }
/*    
    private void createMergeMetadata() {
    	FileSystem fs = hadoopUtil.getFs();
    	String wsId = (String)springRedisTemplateUtil.valueGet("WS_ID");
		...
		if(null != exportSingle) {
			datasetId = exportSingle.get(0).getDatasetIds().get(0);
		}
		else if(null != exportMulti) {
			datasetId = exportMulti.get(0).getDatasetIds().get(0);
		}
...
*/
    private void mergeActions() {
    	...
    	for(int i = 0, len = nodeCount; i < len; i++) {
    		if(0 == i) {
    			contentList = gson.fromJson((String)springRedisTemplateUtil.valueGet("ACTIONS_" + i), new TypeToken<List<JsonArray>>() {}.getType());
    		}
    		else {
    			contentList.addAll(gson.fromJson((String)springRedisTemplateUtil.valueGet("ACTIONS_" + i), new TypeToken<List<JsonArray>>() {}.getType()));
    		}
    	}
    	
    	JsonArray gsonArray = new JsonArray();
    	for(int i = 0, len = contentList.size(); i < len; i++) {
    		gsonArray.addAll(contentList.get(i).deepCopy());
    	}
...
/*    
    private void mergeMetadata() {
    	...
    	for(int i = 0, len = nodeCount; i < len; i++) {
    		if(0 == i) {
    			rowMetadataList = gson.fromJson((String)springRedisTemplateUtil.valueGet("METADATA_" + i), new TypeToken<List<JsonArray>>() {}.getType());
    		}
    		else {
    			rowMetadataList.addAll(gson.fromJson((String)springRedisTemplateUtil.valueGet("METADATA_" + i), new TypeToken<List<JsonArray>>() {}.getType()));
    		}
    	}
    	
    	JsonArray mergeMeta = dprepMetaUtil.mergeMetadata(rowMetadataList);
    	
    	// 하둡 저장
    	String wsId = (String)springRedisTemplateUtil.valueGet("WS_ID");
    	
    	FileSystem fs = hadoopUtil.getFs();
    	hadoopUtil.write(fs, gson.toJson(mergeMeta), hadoopResultRegBasePath, wsId, "metadata.json");
